...
ames %>%
  ...
  ggplot(ames, mapping = aes(x=`Category Name`, y = total_vol, fill=`Category Name`)) + geom_col() + coord_flip() + xlab("Volume Gallons") + ylab('Liqour') + ggtitle('Liqour category by volume') + theme(legend.position = "none")
...
```
  ...
  Based on the output above it seems like an increase in sales during the summer months like July and August. 
...
```
  ...
  Based on the output above it seems like there is a large amount of liqour sold during home games. Especially during the month of August there is a very large amount of alcohol sold and this does make sense as this is the start of the football season so you can imagine many people going to the games and tailgating. 
